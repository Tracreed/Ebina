...
use magick_rust::{MagickWand, magick_wand_genesis, DrawingWand, PixelWand};
...
async fn gen_image(user: &Box<osu_v2::user::User>, usermode: String) {
	...
	avatar.read_image(format!("./{}", &user.id).as_str()).unwrap();
	...
	add_text(&mut wand, &user.statistics.level.current.to_string().as_str(), 291.0, 788.0, "Torus", 35.0, "white");
	add_text(&mut wand, format!("{:.2}%", user.statistics.hit_accuracy).as_str(), 1181.0, 140.0, "Torus", 35.0, "white");
	let pp = user.statistics.pp as u64;
	add_text(&mut wand, pp.to_formatted_string(&Locale::en).as_str(), 1594.0, 140.0, "Torus", 35.0, "white");
	if user.statistics.global_rank.is_some() {
		add_text(&mut wand, format!("#{}", user.statistics.global_rank.unwrap().to_formatted_string(&Locale::en)).as_str(), 1444.5, 269.0 + 10.0, "Torus", 35.0, "white");
	} else {
		add_text(&mut wand, "-", 1444.5, 264.0 + 10.0, "Torus", 35.0, "white");
	}

	if user.statistics.rank.country.is_some() {
		add_text(&mut wand, format!("#{}", user.statistics.rank.country.unwrap().to_formatted_string(&Locale::en)).as_str(), 1444.5, 411.0 + 10.0, "Torus", 35.0, "white");
	} else {
		add_text(&mut wand, "-", 1444.5, 411.0 + 10.0, "Torus", 35.0, "white");
	}

	if user.statistics.total_score > 0 {
		add_text(&mut wand, user.statistics.total_score.to_formatted_string(&Locale::en).as_str(), 1444.5, 551.0 + 10.0, "Torus", 35.0, "white");
	} else {
		add_text(&mut wand, "-", 1444.5, 551.0 + 10.0, "Torus", 35.0, "white");
	}

	if user.statistics.maximum_combo > 0 {
		add_text(&mut wand, user.statistics.maximum_combo.to_formatted_string(&Locale::en).as_str(), 1699.5, 691.0 + 10.0, "Torus", 35.0, "white");
	} else {
		add_text(&mut wand, "-", 1699.5, 691.0 + 10.0, "Torus", 35.0, "white");
	}

	if user.statistics.play_count > 0 {
		add_text(&mut wand, user.statistics.play_count.to_formatted_string(&Locale::en).as_str(), 1209.0, 690.0 + 10.0, "Torus", 35.0, "white");
	} else {
...
fn add_text(wand: &mut MagickWand, text: &str, x: f64, y: f64, font: &str, font_size: f64, color: &str) {
